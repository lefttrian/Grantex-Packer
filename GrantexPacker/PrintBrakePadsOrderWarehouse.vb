...
Public Class PrintBrakePadsOrderWarehouse
    ...
    Private Sub bporder_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        'TODO: This line of code loads data into the 'TESTFINALDataSet.BPORDER' table. You can move, or remove it, as needed.
        Cursor.Current = ExtCursor1.Cursor
        ReportViewer1.LocalReport.ReportPath = "\\192.1.1.7\common\ΤΡΙΑΝΤΑΦΥΛΛΟΥ ΛΕΥΤΕΡΗΣ\Grantex Packer\Reports\BrakePadsOrderWarehouse.rdlc"
...
where ftrid=" + ftrid.ToString + " and substring(m.code,1,3) in ('102','202') group by ftrid,m.mnfid) b on b.ftrid=f.id
                    WHERE F.ID=" + ftrid.ToString
        Dim cmd2 As String = "select * from [dbo].[ftr_checkedby_report](" + ftrid.ToString + ",1)"
        Using COM As New SqlCommand(cmd, updconn)
            Using DT As New DataTable
                ...
                Using com2 As New SqlCommand(" select dbo.[ftr_recipients_report](" + ftrid.ToString + "," + orderphase.ToString + ",1)", updconn)
                ...
                Me.BPORDERapothTableAdapter.Fill(Me.TESTFINALDataSet.BPORDERapoth, ftrid, prodonly)
    ...
    Private Sub ReportViewer1_PrintingBegin(sender As Object, e As ReportPrintEventArgs) Handles ReportViewer1.PrintingBegin
        Dim txt As String = "Εντολή Δισκοφρένων (Μορφή Αποθήκης), " + Form1.activeuser + ", " + DateTime.Now.ToString("yyyy/MM/dd HH:mm") + Environment.NewLine
        ...
        Using ut As New PackerUserTransaction
            ut.WriteEntry(Form1.activeuserid, 25, ftrid)
        End Using
    End Sub
End Class