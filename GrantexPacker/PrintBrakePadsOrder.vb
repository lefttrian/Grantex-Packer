...
Public Class PrintBrakePadsOrder
    ...
    Private Sub bporder_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        ReportViewer1.LocalReport.ReportPath = "\\192.1.1.7\common\ΤΡΙΑΝΤΑΦΥΛΛΟΥ ΛΕΥΤΕΡΗΣ\Grantex Packer\Reports\BrakePadsOrder.rdlc"
...
where ftrid=" + ftrid.ToString + " and substring(m.code,1,3) in ('102','202') group by ftrid,m.mnfid) b on b.ftrid=f.id
        ...
        Using COM As New SqlCommand(cmd, updconn)
            Using DT As New DataTable
                ...
                Me.BPORDERTableAdapter.Fill(Me.TESTFINALDataSet.BPORDER, ftrid, prodonly)
    ...
    Private Sub ReportViewer1_PrintingBegin(sender As Object, e As ReportPrintEventArgs) Handles ReportViewer1.PrintingBegin
        Dim txt As String = "Εντολή Δισκοφρένων (Μορφή Παραγωγής), " + Form1.activeuser + ", " + DateTime.Now.ToString("yyyy/MM/dd HH:mm") + Environment.NewLine
        ...
        Using ut As New PackerUserTransaction
            ut.WriteEntry(Form1.activeuserid, 24, ftrid)
        End Using
    End Sub
End Class