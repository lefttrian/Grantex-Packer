...
Public Class PrintBrakeLiningsOrderWarehouse
    ...
    Private Sub BLORDER_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        'TODO: This line of code loads data into the 'TESTFINALDataSet.BLORDER' table. You can move, or remove it, as needed.
        Cursor.Current = ExtCursor1.Cursor
        ReportViewer1.LocalReport.ReportPath = "\\192.1.1.7\common\ΤΡΙΑΝΤΑΦΥΛΛΟΥ ΛΕΥΤΕΡΗΣ\Grantex Packer\Reports\BrakeLiningsOrderWarehouse.rdlc"
        ...
        Dim cmd As String = "SELECT FTRTIME ,C.FATHERNAME,
                    ...
                    salesman s on s.id=f.colidsalesman
                    left join
...
where ftrid=" + ftrid.ToString + " and substring(m.code,1,3) in ('101','104','118') group by ftrid,m.mnfid) b on b.ftrid=f.id
                    WHERE F.ID=" + ftrid.ToString
        Dim cmd2 As String = "select * from [dbo].[ftr_checkedby_report](" + ftrid.ToString + ",2)"
        Using COM As New SqlCommand(cmd, updconn)
            Using DT As New DataTable
                ...
                Using com2 As New SqlCommand(" select dbo.[ftr_recipients_report](" + ftrid.ToString + "," + orderphase.ToString + ",2)", updconn)
                ...
                Me.BLORDERapothTableAdapter.Fill(Me.TESTFINALDataSet.BLORDERapoth, ftrid, prodonly)
    ...
    Dim prodonly As String
    Dim ftrid As Integer
    Public Sub New(ByVal value As Integer, ByVal value2 As Integer, Optional p As String = "")
        ftrid = value
        orderphase = value2
        prodonly = p
        InitializeComponent()
    ...
    Private Sub ReportViewer1_PrintingBegin(sender As Object, e As ReportPrintEventArgs) Handles ReportViewer1.PrintingBegin
        ...
        Dim txt As String = "Εντολή Φερμουίτ (Μορφή Αποθήκης), " + Form1.activeuser + ", " + DateTime.Now.ToString("yyyy/MM/dd HH:mm") + Environment.NewLine
        ...
        Using ut As New PackerUserTransaction
            ut.WriteEntry(Form1.activeuserid, 22, ftrid)
        End Using
    End Sub
End Class